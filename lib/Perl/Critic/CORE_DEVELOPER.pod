...
considered definitive. For the Subversion root as of
revision 3812, you need the following in addition to B<all> optional
modules for Perl::Critic itself.
...
In the absence of C<Test::Memory::Cycle> and C<Test::Without::Module>,
the relevant tests are simply skipped. In the absence of the other
modules, the tests die horribly. Of course, either way they do not get
run, so the difference is mainly one of aesthetics.
...
In addition to the above, C<Perl::Critic> revision 3812 requires
L<PPIx::Utilities|PPIx::Utilities> to be installed before C<Build.PL>
can be executed. It is anticipated that this behavior will go away in
