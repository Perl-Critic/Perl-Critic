...
or C<ProhibitSomething>.  Each Policy is listed here with its default
...
Write C<split /-/, $string> instead of C<split '-', $string> [Severity 3]
...
Write C<< my $error = close $fh; >> instead of C<< close $fh; >> [Severity 3]
...
Use C<[abc]> instead of C<a|b|c> [Severity 5]
...
Use only C<//> or C<{}> to delimit regexps [Severity 5]

=head2 L<Perl::Critic::Policy::RegularExpressions::RequireBracesForMultiline>

Use C<{> and C<}> to delimit multi-line regexps [Severity 5]
...
Magic variables should be assigned as "local" [Severity 2]
