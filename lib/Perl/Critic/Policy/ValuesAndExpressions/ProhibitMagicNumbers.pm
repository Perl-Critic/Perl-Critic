...
my $UNSIGNED_NUMBER =
    qr/
        ...
        |   [$PERIOD] \d+             # .3, .7, etc.
    /xms;
my $SIGNED_NUMBER = qr/ [-+]? $UNSIGNED_NUMBER /xms;
my $RANGE =
    qr/
        \A
        ($SIGNED_NUMBER)
...
my $SPECIAL_ARRAY_SUBSCRIPT_EXEMPTION = -1; ## no critic (ProhibitMagicNumbers)
...
plans.  Uses of the L<Readonly> module are obviously valid.
Declarations of C<$VERSION> package variables are permitted.
