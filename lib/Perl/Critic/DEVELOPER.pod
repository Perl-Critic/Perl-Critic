...
argument: a reference to a hash of the configuration for the Policy.
This method is only called if the user's configuration has enabled the
policy.  It returns a boolean stating whether the Policy should
continue to be enabled.  Generally, the only reason to return
C<$FALSE> is when some external requirement is missing.  For example,
...
this:
  ...
  use Perl::Critic::Utils qw{ :booleans :data_conversion };
...
used, with user-supplied parameter values valididated and placed into
