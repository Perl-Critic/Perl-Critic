...
object, passing in the description, explanation, and a reference to
the PPI element that caused the violation.  And that's all there is to
it!
...
superclass constructor does.  The reason to actually create a
constructor is to do something with C<%config>.
...
invoke C<_finish_standard_initialization()> on the Policy before
accessing the parsed parameter values.
