...
call it C<"Perl::Critic::Policy::BuiltinFunctions::RequireBlockGrep">.
...
Most policies don't need to override the new() method provided by
L<Perl::Critic::Policy>.  However, if your policy is configurable via
F<.perlcriticrc> you will need to implement a
C<supported_parameters()> method and/or override the constructor to
examine the C<%config> values.  See
L</MAKING YOUR POLICY CONFIGURABLE> below.

Next, we define the C<default_severity()> method, which should return
a scalar value indicating the severity of violating this Policy.
...
and/or cause bugs.  Level 1 is for things that are highly subjective
or purely cosmetic.  The L<Perl::Critic::Utils> package exports
several severity constants that you can use here.
...
Policies that ship with Perl::Critic have a C<"core"> theme.  And
since this Policy comes directly from Damian's "Perl Best Practices"
book, this Policy should be a member of the C<"pbp"> theme.
...
only one PPI::Document element, such a Policy would only be invoked
once per file.
...
(C<$self>), but you already knew that!]
...
being passed to it.  Every PPI element is linked to its siblings,
parent, and children (if it has any).  Since those siblings could just
be whitespace, we use the C<snext_sibling()> to get the next
code-sibling (the 's' in C<snext_sibling> stands for 'significant').
...
One last thing -- When you import L<Perl::Critic::Violation>, it
extracts the DESCRIPTION section from the POD in your Policy module.
That text can displayed in the diagnostic output when the Violations
objects are stringified.  So please include a DESCRIPTION section in
the POD for your Policy.  It should succinctly describe the behavior
and motivation for your Policy and include a few examples of both good
and bad code.  Here's an example:
...
L<perlcritic/CONFIGURATION> for the details of how a user specifies 
...
The constructor for a Policy receives two arguments: the standard
class name argument, plus a hash of the configuration for the Policy;
this is a regular hash, and not a reference to one.  Unfortunately,
Policies are not required to call the super constructor; however, in
order to work correctly, a Policy must be a blessed hash.  An example
minimal constructor:
...
Actually using the above code is pretty useless since this is what the
Policy constructor does.  The reason to actually create a constructor
is to do something with C<%config>.
...
prefixed with an underscore.  E.g.
  ...
  sub new {
      ...
      $self->{_selections} = $config{selections};
...
this:
  ...
  sub new {
      ...
      $self->{_foo} = $config{foo};
      $self->{_factor} = $config{factor};
      $self->{_selections} = [ words_from_string( $config{selections} ) ];
...
the assignment to look like
  ...
  $self->{_selections} = [ map { $_ + 0 } words_from_string($config{selections}) ];
...
like this:
  ...
  $self->{_selections} = { hashify( words_from_string( $config{selections} ) ) };
...
supports.  So, for an non-configurable Policy, this should look like

  sub supported_parameters { return (); }

For the example being used in the constructor section above, this
would be
...
From this C<Perl::Critic> can emit a better F<.perlcriticrc>
prototype.
...
as a Policy author easier; you still need to implement a constructor
to access any configuration that the user has specified.  To have the
configuration automatically handled for you, you need to declare how
your parameters act by specifying a value for their C<behavior>.  For
example, the following declares that a parameter allows the user to
choose from five specific values and that the user can select any
combination of them:
...
Implemented in L<Perl::Critic::PolicyParameter::StringBehavior>.
...
Implemented in L<Perl::Critic::PolicyParameter::BooleanBehavior>.
...
Implemented in L<Perl::Critic::PolicyParameter::IntegerBehavior>.

The value is validated against C<m/ \A [-+]? [1-9] \d* \z /xms>.
...
Implemented in L<Perl::Critic::PolicyParameter::StringListBehavior>.

The values will be derived by splitting the input string on blanks.
(See L<Perl::Critic::Utils/words_from_string>.) The parameter will be
...
Implemented in L<Perl::Critic::PolicyParameter::EnumerationBehavior>.
...
Example:
  ...
  sub supported_parameters {
      return (
          {
              name               => 'an_enumeration',
              description        => 'An example enumeration.',
...
opposed to using a constructor is that it allows you to use a behavior
to provide its extra functionality and it provides a means for a
C<Perl::Critic> configuration program, e.g. an IDE that integrates
C<Perl::Critic>, to validate your parameter as the user
modifies its value.
      ...
      if ($config_string) {
          if (not -r $config_string) {
              die q{Invalid value for },
                  $parameter->get_name(),
                  qq{: $config_string is not readable.\n};
          }
...
Note that, if the value for the parameter is not valid, C<die> is used
instead of C<confess> because the problem is not a bug in the code but
a problem with user input.


=head2 Using Both C<supported_parameters()> and a Constructor
...
multiple parameters.  In such situations, you will need to create a
constructor.  If you want to take advantage of the supplied parameter
handling from within the constructor, there's a problem because
L<Perl::Critic> can't do its initialization of a Policy until the
Policy has been created.  (Due to the lack of a requirement for a
Policy to invoke the super-constructor.)  The way around this is to
invoke C<_finish_standard_initialization()> on the Policy before accessing the
...
This is done like so:
  ...
  sub new {
      ...
      # do additional setup that requires the parameter values

      return $self;
  }

An example of a Policy that does this is
