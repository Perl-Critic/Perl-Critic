...
page numbers in I<PBP>.

  my $desc = q{Expression form of "grep"};
  my $expl = [169];

Most policies don't need to override the C<new()> method provided by
...
examine the C<%config> values.  See L</MAKING YOUR POLICY CONFIGURABLE>
below.

Next, we define the C<default_severity()> method, which must return an
intger indicating the severity of violating this Policy.  Severity
...
since this Policy comes directly from I<PBP>, this Policy should be a
member of the C<"pbp"> theme.
