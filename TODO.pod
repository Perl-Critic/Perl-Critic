...
=item * A function that splits up a PPI::Node's descendants based upon package.
...
Calling the function on a PPI::Document for the above should return a hash of
arrays of PPI::Nodes.  The result should look something like this, using
multi-line string literals (RestrictLongStrings violations!) for the actual
...
=item * Run-on single line annotations

The problem here is that for something like:

  my $foo =
    '$bar'; ## no critic (RequireInterpolationOfMetachars) 

The logic that detects single line annotations for simple statements does so
by finding the previous significant sibling to the comment, and seeing if that
sibling is on the same line. But this logic fails for the above sample,
because the PPI document makes the comment a sibling to the entire statement,
which starts on the previous line.

There is a less fulsome TODO on this in F<lib/Perl/Critic/Annotation.pm> at
the point where the fix needs to be applied.

When this gets fixed the comment at or about line 105 in
F<lib/Perl/Critic/Policy/Variables/ProhibitPunctuationVars.pm> needs to be
removed, and the semicolon it comments put in the natural place, before the
annotation.
