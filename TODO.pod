...
thread.  Although there's now C<Policy::prepare_to_scan_document()>, given
perl's thread data sharing model, this shouldn't be an issue.
...
=item * ControlStructures::ProhibitPostfixControls

Look for the do {} while case and change the explanation to point to page 123
when it is found.  RT #37905.
