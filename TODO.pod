...
=item * Don't kill entire run if PPI::Document cannot be instantiated.

Just say that the individual file is bad and skip to the next one.
...
Detect a variable that is declared, but never used.
...
=item * Document maximum_violations_per_document in DEVELOPER.pod.


=item * Generate PolicySummary.pod instead of maintaining it.

Move all the descriptions to the NAME section of each Policy and generate
PolicySummary.pod from that.  Also can be used to enhance ProfilePrototype
output.


=item * Replace policy configuration hashes with a PolicyConfig object.

Encapsulate parameter parsing.
