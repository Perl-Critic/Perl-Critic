...
InputOutputRequireCheckedOpen.  Needs to take into account the Fatal
and Fatal::Exception modules.
...
=item * Move all the parameters construction stuff from PolicyFactory to Policy.

We can do this because we're now going to require all policies that use the
parameters stuff to invoke the super constructor.
