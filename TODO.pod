...
=item * CodeLayout::RequireConsistentNewlines
...
NOTE: this is impossible to implement as of the current PPI (v1.118)
because PPI fixes all newlines before tokenization as an
implementation expedient.  The PPI docs state that newline preserving
is on the wishlist.
...
CodeLayout::RequireConsistentNewlines impossible to implement under
PPI.
