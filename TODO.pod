...
Can't be done as a Policy because any line that violated it would disable it.
...
Strictly, requires parsing the regexp to find the escapes.
...
People writing ASCII-only code will want to disable this policy or put in ##
no critic flags.

Strictly, requires parsing the regexp to find the enumerations.
...
Requires parsing the regexp to find the captures.

=item * RegularExpressions::ProhibitComplexRegexes (p261)

If regexp is longer than N characters/lines, require it be split into C<qr//>
pieces.

Even if the x modifier is used and the regex is split across multiple lines?
Perhaps don't count comments toward the character limit.

=item * RegularExpressions::ProhibitSingleCharAlternation (p265)

Make an exception for qr/ [ ] /x.

Strictly, requires parsing the regexp.
