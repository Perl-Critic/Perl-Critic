...
=item * TBD::ProhibitLabelsWithSpecialBlockNames

BEGIN { <...code...> }
is a BEGIN block that gets executed at compile time.

BEGIN: { <...code...> }
is a labeled block that gets executed during run time.

I suspect that any block named BEGIN, END, INIT or CHECK is an error.
This should be a standard rule.
