...
Chris Dolan is in the process of implementing the ideas below (but don't let
that stop you from contributing!!!).  That work is supported by a Perl
Foundation grant.  More details about the grant:
...
The former really means

  @list = 1; 2; 3;

See also ValuesAndExpressions::ProhibitCommaSeparatedStatements
...
Flag assignments to magic variables that are not declared local.  Detect both
the punctation forms and the English forms.  Don't flag C<$_>.  Other
exceptions?
...
InputOutput::RequireCheckedOpen and InputOutput::RequireCheckedClose.  Needs
to take into account the Fatal and Fatal::Exception modules.
...
Recommend C<local $/ = undef> instead.
...
=item * RegularExpressions::ProhibitComplexRegexes (p261)

If regexp is longer than N characters/lines, require it be split
into C<qr//> pieces.

Even if the x modifier is used and the regex is split across
multiple lines?
...
=item * Make parameter declarations at compile time instead of at run time.

=item * Have the Policy import() inject accessors for parameters into subclasses.

=item * Switch to using exceptions instead of die and confess.

=item * Restore tri-state-edness to Behavior::Boolean.

=item * Move package globals, e.g. P::C::Violation::FORMAT, to instance variables on Perl::Critic.

This applies regardless of whether the variable was declared with C<my> or
C<our>.

=item * Have PolicyFactory look for Configuration exceptions coming out of Policy constructors.
