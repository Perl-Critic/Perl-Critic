...
=item * Move %FORMATS from `perlcritic` into Perl::Critic::Utils so that
...
=item * Enhance P::C::critique() to accept file names, directories, or code strings (as refs) just like `perlcritic` does now.
